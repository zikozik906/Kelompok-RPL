			...
			<div class="grid-body">
				<table id="dataTables1" class="table table-hover display" cellspacing="0" width="100%">
					...
					<tbody>
						<?php 
							$no 		=	1;
							$alat 		=	mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM alat ");
						...
						<tr>
							...
							<td>
								<?php
									echo $data['alat_jumlah']; 
								?>
							</td>
							<td>
								<?php 
									echo $data['alat_rusak'];
								?>
							</td>	
							<td><?php echo $data['alat_satuan']; ?></td>
							<?php 
								if (isset($_SESSION['access'])) {
									if ($_SESSION['access'] == 'admin') {
							?>
							...
							<?php
										...
										$jumlah = $data['alat_jumlah'];
										$rusak 	= $data['alat_rusak']; 
							?>
							<td class="btn-info">
								<center>
